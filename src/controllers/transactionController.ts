   ...
   */
   static async verifyAccountData(req: any, res: Response, next: NextFunction) {
    ...
    try {
      ...
      res.status(200).json({error: false, data: response.account_name, message: "verification success"})
    } catch (err) {
      next(err);
    }
  }
...
static async sendMoneyToUser(req: any, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (sendMoney) {
      res.status(200).json({error: false, message: "money sent successfully"})}
  } catch (err) {
    next(err);
  }
}
...
static async withDrawFromWallet(req: any, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (withdrawal) {
      res.status(200).json({error: false, message: "require third party API"})
    }
  } catch (err) {
    next(err);
  }
 ...
 */
  ...
  static async getTransactionByUserId(
    ...
    try {
      ...
      const transaction = Transactions.findOne({ user: userId });
